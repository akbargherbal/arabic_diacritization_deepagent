...
def _iter_valid_records(path: pathlib.Path):
    ...
    for i, raw_line in enumerate(lines):
        ...
        try:
            ...
            if is_last:
                print(f"[!] Warning: truncated trailing line in {path.name}, "
                      f"likely from an interrupted write — skipping it.")
            else:
                print(f"[!] Warning: corrupt line {i} in {path.name} (not the "
...
def commit_verse_tool(
    ...
    with _commit_lock:
        ...
        if verse_id in _seen_verse_ids:
            return {"committed": False, "reason": "duplicate verse_id already committed",
                     "duplicate": True}
